...
import {
  ...
  Users, ShieldAlert, MessageCircle, Inbox
} from "lucide-react";
...
export default function MessagingDrawer({
  ...
  const totalUnread = useMemo(() => rawConversations.reduce((sum: number, conv: any) => sum + (conv._count?.messages || 0), 0), [rawConversations]);
  ...
  }, [socket, isConnected, rawConversations, queryClient]);
  ...
  const renderConversationButton = (conv: any) => {
    ...
    const displayTitle = isClassroomGroup
      ? `${conv.classroom.name} Team`
      : isDM
        ? dmPartner?.name
        : `${conv.child?.firstName} ${conv.child?.lastName}`;
      ...
      >
        ...
        <div className="flex-1 overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-0.5">
            ...
            {lastMsg && (
              <span className={`text-[10px] whitespace-nowrap ml-2 ${hasUnread ? "font-bold text-primary" : "text-muted-foreground"}`}>
  ...
  const getDisplayedConversations = () => {
    switch (activeTab) {
      case "all": return sortedConversations;
  ...
  // A Guardian only ever has child-threads — every conversation they have
  // would match every tab identically, so skip the filtering entirely
  // rather than show tabs that can never do anything.
  const displayedConversations = isGuardian ? sortedConversations : getDisplayedConversations();
            ...
            <div className="flex items-center justify-between p-4 border-b border-border bg-muted/10">
              <div className="flex items-center gap-2">
                {(activeConv || showContacts) && !targetChildId && (
                  <button
                    onClick={() => { setActiveConv(null); setShowContacts(false); }}
              ...
              <div className="flex items-center gap-1">
                {!activeConv && !showContacts && !isGuardian && (
                  <button onClick={() => setShowContacts(true)} className="p-2 rounded-md hover:bg-muted transition-colors text-primary" title="New Message">
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                  <div className="flex border-b border-border">
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold tracking-wide transition-colors ${contactTab === "direct" ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}
                    >
                      <User className="size-3.5" /> Direct
                    </button>
                    <button
                      onClick={() => setContactTab("teams")}
                      className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold tracking-wide transition-colors ${contactTab === "teams" ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}
                    >
                  ...
                  <div className="flex-1 overflow-y-auto p-2">
                    {isLoadingContacts ? (
                      ...
                      contactsData?.classrooms?.length > 0 ? (
                        contactsData.classrooms.map((room: any) => (
                          <button
                            key={room.id}
                        ...
                        <div className="text-center p-8 text-sm text-muted-foreground">No classroom teams available.</div>
                      )
                    ) : (
                      contactsData?.contacts?.length > 0 ? (
                        contactsData.contacts.map((contact: any) => (
                          ...
                          >
                            {contact.image ? (
                              <Image src={contact.image} alt={contact.name} width={40} height={40} className="size-10 rounded-full object-cover shrink-0" />
                            ) : (
                              <div className="flex size-10 items-center justify-center rounded-full bg-primary/10 text-primary shrink-0">
                        ...
                        <div className="text-center p-8 text-sm text-muted-foreground">No contacts available in your network.</div>
                      )
                    )}
                  </div>
                </div>
                  ...
                  {!isGuardian && (
                    <div className="flex overflow-x-auto border-b border-border bg-card z-10 sticky top-0 custom-scrollbar">
                      <button
                        ...
                        className={`min-w-[75px] flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold tracking-wide transition-colors ${activeTab === "all" ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}
                      >
                        <Inbox className="size-3.5" /> All
                      </button>
                      <button
                        onClick={() => setActiveTab("direct")}
                        className={`min-w-[75px] flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold tracking-wide transition-colors ${activeTab === "direct" ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}
                      >
                        <MessageCircle className="size-3.5" /> Direct
                      </button>
                      <button
                        onClick={() => setActiveTab("teams")}
                        className={`min-w-[75px] flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold tracking-wide transition-colors ${activeTab === "teams" ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}
                      >
                        <Users className="size-3.5" /> Teams
                      </button>
                      <button
                        onClick={() => setActiveTab("guardians")}
                        className={`min-w-[90px] flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold tracking-wide transition-colors ${activeTab === "guardians" ? "border-b-2 border-primary text-primary" : "text-muted-foreground hover:text-foreground"}`}
                      >
                    ...
                    {isLoadingConvs ? (
                      ...
                      <div className="flex flex-col h-full items-center justify-center text-center p-8">
                        ...
                        <p className="text-sm font-medium text-foreground">
                          {isGuardian ? "No messages yet." : `No ${activeTab} conversations.`}
                        </p>
                        {isGuardian ? (
                          <p className="mt-2 text-xs text-muted-foreground">
                            Open your child&apos;s page to start a conversation with their teacher.
                          </p>
                        ) : (
                          <button onClick={() => { setShowContacts(true); setContactTab(activeTab === "teams" ? "teams" : "direct"); }} className="mt-4 flex items-center gap-2 text-sm text-primary hover:underline">
