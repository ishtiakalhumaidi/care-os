...
export default function GalleryPageHeader({ childCount }: { childCount: number }) {
  return (
    <>
      {/* Back nav */}
      <motion.div
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5 max-w-lg text-sm leading-relaxed text-muted-foreground">
                Cherished moments captured by teachers. Browse, preview, and download photos of your children.
              </p>
            </div>
          </div>
          {childCount > 0 && (
            <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-2.5 shadow-sm">
              ...
              <span className="text-xs font-medium text-muted-foreground">
                {childCount} {childCount === 1 ? "child" : "children"} linked
              </span>
            </div>
          )}
        </div>
